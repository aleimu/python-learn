...
1文件{	
	...
	find查找{
		...
		#通过自身的参数-exec来将查找到的结果传递给rm进行删除
		find load* -exec rm {} \;		find load* -exec vi {} \;		find load* -exec sh {} \;
		#通过xargs命令，将上一个命令的结果输出，作为rm命令的输入
		find load* |xargs rm
...
3系统{
	...
	定时任务{
		...
		crontab -u topus -l #查看topus用户的定时任务
		crontab -e               # 编辑周期任务
		#分钟  小时    天  月  星期   命令或脚本
...
9 实例{
	...
	expect交互自动登录{
	通过expect ssh.exp 127.0.0.1 huawei执行{
...
2.spawn ssh root@192.168.1.130   
3.root@192.168.1.130s password:   
